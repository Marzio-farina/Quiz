...
/**
 * Script per confrontare le domande di ssfo-quiz-modello7.pdf
...
const modello7QuizPath = path.join(__dirname, 'modello7-quiz-data.json');
const outputPath = path.join(__dirname, 'comparison-modello7-report.json');
...
const modello6Quizzes = modello6Data.quizzes || [];
console.log(`✅ Caricati ${modello6Quizzes.length} quiz da modello6-quiz-data.json`);

// Carica i quiz del modello7 (se esiste)
let modello7Quizzes = [];
if (fs.existsSync(modello7QuizPath)) {
    const modello7Data = JSON.parse(fs.readFileSync(modello7QuizPath, 'utf8'));
    modello7Quizzes = modello7Data.quizzes || [];
    console.log(`✅ Caricati ${modello7Quizzes.length} quiz da modello7-quiz-data.json`);
} else {
    console.log(`⚠️  File modello7-quiz-data.json non trovato. Esegui prima extractModello7Quizzes.js`);
}
...
[existingQuizzes, newQuizzes, modello3Quizzes, modello4Quizzes, modello5Quizzes, modello6Quizzes].forEach((quizzes, index) => {
    const sources = ['quiz-data.json', 'new-quiz-data.json', 'modello3-quiz-data.json', 'modello4-quiz-data.json', 'modello5-quiz-data.json', 'modello6-quiz-data.json'];
...
// Confronta le domande del modello7
function compareQuizzes() {
    if (modello7Quizzes.length === 0) {
        console.log('\n⚠️  Nessuna domanda del modello7 da confrontare.');
        console.log('   Esegui prima: node extractModello7Quizzes.js');
        return;
    }
    
    const comparison = {
        source: 'ssfo-quiz-modello7.pdf',
        totalModello7Questions: modello7Quizzes.length,
    ...
    modello7Quizzes.forEach((q) => {
        ...
        if (existingMatches.length > 0) {
            comparison.duplicates.push({
                modello7Id: q.id,
                modello7Question: q.question.substring(0, 150),
    ...
    console.log(`Totale domande in modello7: ${comparison.totalModello7Questions}`);
    ...
    console.log(`Totale domande in modello6-quiz-data.json: ${comparison.totalModello6Questions}`);
